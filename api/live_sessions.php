...
{
    $row['id'] = (int) $row['id'];
    $row['course_id'] = isset($row['course_id']) ? ($row['course_id'] === null ? null : (int) $row['course_id']) : null;
    if (!array_key_exists('course_title', $row)) {
...
if ($method === 'GET') {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($user['role'] === 'admin') {
            $stmt = $mysqli->prepare('SELECT ls.id, ls.course_id, c.title AS course_title, ls.title, ls.description, ls.stream_url, ls.starts_at, ls.ends_at, ls.created_at FROM live_sessions ls LEFT JOIN courses c ON c.id = ls.course_id WHERE ls.id = ? LIMIT 1');
            if (!$stmt) {
                error_response('无法获取直播课');
            }
            $stmt->bind_param('i', $sessionId);
        } else {
            $stmt = $mysqli->prepare('SELECT ls.id, ls.course_id, c.title AS course_title, ls.title, ls.description, ls.stream_url, ls.starts_at, ls.ends_at, ls.created_at FROM live_sessions ls LEFT JOIN courses c ON c.id = ls.course_id LEFT JOIN user_courses uc ON uc.course_id = ls.course_id AND uc.user_id = ? LEFT JOIN user_live_sessions uls ON uls.live_session_id = ls.id AND uls.user_id = ? WHERE ls.id = ? AND (ls.course_id IS NULL OR uc.user_id IS NOT NULL OR uls.user_id IS NOT NULL) LIMIT 1');
            if (!$stmt) {
                error_response('无法获取直播课');
            }
            $stmt->bind_param('iii', $user['id'], $user['id'], $sessionId);
        }
        $stmt->execute();
    ...
    if ($user['role'] === 'admin') {
        $sql = 'SELECT ls.id, ls.course_id, c.title AS course_title, ls.title, ls.description, ls.stream_url, ls.starts_at, ls.ends_at, ls.created_at FROM live_sessions ls LEFT JOIN courses c ON c.id = ls.course_id ORDER BY COALESCE(ls.starts_at, ls.created_at) ASC, ls.id ASC';
        $result = $mysqli->query($sql);
        if (!$result) {
            error_response('无法获取直播课列表');
        }
    } else {
        $stmt = $mysqli->prepare('SELECT DISTINCT ls.id, ls.course_id, c.title AS course_title, ls.title, ls.description, ls.stream_url, ls.starts_at, ls.ends_at, ls.created_at FROM live_sessions ls LEFT JOIN courses c ON c.id = ls.course_id LEFT JOIN user_courses uc ON uc.course_id = ls.course_id AND uc.user_id = ? LEFT JOIN user_live_sessions uls ON uls.live_session_id = ls.id AND uls.user_id = ? WHERE ls.course_id IS NULL OR uc.user_id IS NOT NULL OR uls.user_id IS NOT NULL ORDER BY COALESCE(ls.starts_at, ls.created_at) ASC, ls.id ASC');
        if (!$stmt) {
            error_response('无法获取直播课列表');
        }
    ...
    $rawCourseId = $input['course_id'] ?? null;
    $courseId = is_null($rawCourseId) || $rawCourseId === '' ? null : (int) $rawCourseId;
    ...
    $stmt = $mysqli->prepare('SELECT ls.id, ls.course_id, ls.title, ls.description, ls.stream_url, ls.starts_at, ls.ends_at, c.title AS course_title FROM live_sessions ls LEFT JOIN courses c ON c.id = ls.course_id WHERE ls.id = ? LIMIT 1');
    if (!$stmt) {
    ...
    $courseId = array_key_exists('course_id', $input)
        ? (is_null($input['course_id']) || $input['course_id'] === '' ? null : (int) $input['course_id'])
        : (isset($current['course_id']) ? (int) $current['course_id'] : null);
    ...
    $courseTitle = $current['course_title'] ?? null;

    if (($current['course_id'] ?? null) !== $courseId) {
        if (!is_null($courseId) && $courseId > 0) {
            $stmt = $mysqli->prepare('SELECT id, title FROM courses WHERE id = ? LIMIT 1');
            if (!$stmt) {
                error_response('无法验证课程');
