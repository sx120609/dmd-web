...
云盘存储目录权限说明：
- 默认存储在项目根目录的 `uploads/files`，若服务器根目录不可写，请手动创建该目录并赋予 PHP 进程写权限（例如 chown/chmod）。
- 也可在 `config.php` 内设置 `storage.cloud_dir` 为可写的绝对路径或相对路径（相对路径基于项目根目录）。
...
- 前端通过 CDN 加载 Bootstrap 5 与 Plyr，如需离线或内网部署请将相关静态资源下载到本地并更新引用地址。
